...
CONCEPT_REQUIRES_(TextErrorPolicy<TEP>())>
error_policy tep_index();
template<>
constexpr error_policy tep_index<text_strict_error_policy>() {
    return strict;
}
template<>
constexpr error_policy tep_index<text_permissive_error_policy>() {
    return permissive;
}
    ...
    error_policy ep)
{
    ...
    for (size_t i = 0; i < ecums.size(); ++i) {
        ...
        if (ecums[i].c_or_st.has_character()) {
            ...
            typename make_unsigned<code_point_type_t<character_type>>::type cp =
                c.get_code_point();
            cerr << "    character:" << endl;
        ...
        if (! expected_code_units.empty()) {
            ...
            for (typename make_unsigned<code_unit_type>::type cu : expected_code_units[0]) {
                cerr << "      ["
    ...
    error_policy ep)
{
    ...
    for (size_t i = 0; i < ecms.size(); ++i) {
        cerr << "  [" << dec << setw(2) << setfill(' ') << i << "]:" << endl;
        cerr << "    code units:" << endl;
        for (typename make_unsigned<code_unit_type>::type cu : ecms[i].code_units) {
            cerr << "      ["
                 << dec << setw(2) << setfill(' ') << cu_index++
                 << "]:"
...
template<typename ET, typename CUT = code_unit_type_t<ET>,
CONCEPT_REQUIRES_(TextEncoding<ET>(), CodeUnit<CUT>())>
    ...
    CUT actual_cu)
{
    ...
    ++error_count;
    display_expected_code_unit_map_sequence<ET>(ecums, ep);
    typename make_unsigned<CUT>::type expected_unsigned_cu = expected_cu;
    typename make_unsigned<CUT>::type actual_unsigned_cu = actual_cu;
...
template<
    typename ET,
    typename CT = character_type_t<ET>,
    typename CPT = code_point_type_t<CT>,
CONCEPT_REQUIRES_(TextEncoding<ET>(), Character<CT>(), CodePoint<CPT>())>
    ...
    CT actual_c)
{
    ...
    ++error_count;
    display_expected_character_map_sequence<ET>(ecms, ep);
    typename make_unsigned<CPT>::type expected_unsigned_cp = expected_c.get_code_point();
    typename make_unsigned<CPT>::type actual_unsigned_cp = actual_c.get_code_point();
    ...
    CT expected_c)
{
    ...
    typename make_unsigned<CPT>::type expected_unsigned_cp =
        expected_c.get_code_point();
    ...
    CT actual_c)
{
    ...
    typename make_unsigned<CPT>::type actual_unsigned_cp =
        actual_c.get_code_point();
...
template<
    typename ET,
    typename CUT = code_unit_type_t<ET>,
    typename CT = character_type_t<ET>,
CONCEPT_REQUIRES_(TextEncoding<ET>(), CodeUnit<CUT>(), Character<CT>())>
...
template<
    ...
    typename CT = character_type_t<ET>,
    typename CUT = code_unit_type_t<ET>,
CONCEPT_REQUIRES_(TextEncoding<ET>(), Character<CT>(), CodeUnit<CUT>())>
...
template<
    typename ET,
    typename TEP,
CONCEPT_REQUIRES_(TextEncoding<ET>(), TextErrorPolicy<TEP>())>
...
void test_utf32_encoding() {
    ...
    expected_code_unit_map_sequence<ET> encode_tests[] = {
        ...
        { {
            CT{U'A'}, {
                /*strict*/     { { CUT{0x00000041} } },
                /*permissive*/ { { CUT{0x00000041} } }
            }
          }, {
            CT{0x0000D800}, {
                /*strict*/     {},
            ...
            CT{U'C'}, {
                /*strict*/     { { CUT{0x00000043} } },
                /*permissive*/ { { CUT{0x00000043} } }
            }
        } },
        { {
            CT{U'A'}, {
