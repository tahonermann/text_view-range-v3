...
class caching_cursor
{
    ...
    using cache_view_type =
        basic_view<typename std::deque<value_type>::const_iterator>;
...
public:
    ...
    public:
        ...
        -> decltype(this->get().cached_range())
        {
            return this->get().cached_range();
        }

        auto look_ahead_range() const noexcept
        -> decltype(this->get().look_ahead_range())
        {
    ...
    cache_view_type cached_range() const noexcept {
        return make_basic_view(
                   current_data->cache.begin(),
                   current_data->cache.begin()+position);
    }

    cache_view_type look_ahead_range() const noexcept {
        return make_basic_view(
                   current_data->cache.begin()+position,
